...
def load_strokes(data_dir, hps):
    ...
    if isinstance(hps.data_set, list):
        ...
        print("isinstance dataset : ", datasets)
    else:
        datasets = hps.data_set.split(",")
        print("isnotinstance dataset : ", datasets)
    ...
    for dataset in datasets:
        if data_dir.startswith('http://') or data_dir.startswith('https://'):
            ...
            data = np.load(BytesIO(response.content), encoding='latin1', allow_pickle=True)
        else:
